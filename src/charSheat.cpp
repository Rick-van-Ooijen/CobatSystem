...
void CharSheat::_bind_methods() {

	ClassDB::bind_method(D_METHOD("InitializeValues", "input"), &CharSheat::InitializeValues);
	ClassDB::bind_method(D_METHOD("PrintData"), &CharSheat::PrintData);
...
void CharSheat::InitializeValues(String arg) {
		...
		if (line == "str") {state = 1;}
			...
			if (pos != std::string::npos) {
				...
				switch(state) {
					...
					UtilityFunctions::print("adding sum shit");
					break;
					case 2:
					{
						num newNum = num();
					...
					break;
					default:
					UtilityFunctions::print("Error: line which is not a valid attribute processed in character sheat initialization");
				}
...
void CharSheat::_process(double delta) {
